...
const pages = [
    ...
    blocks: [
      { kind: "p", text: "Click a unit and the whole line takes the order. Clicking cycles advance, then halt, then reform. One more click advances again." },
      {
        ...
        items: [
          ...
          "Halt: hold still.",
          "Reform: move at half speed. The front stops until the rear walks into a perfect line, and a stopped unit still shoots. A perfect line keeps walking together at half speed. A unit that joins from behind takes the reform order, unless that line already fills every row. A full line keeps its order. A unit walking through it keeps its own order and does not halt, reform, or charge that line. An order reaches a line with an open row only from a unit standing on that open row, never from one sharing a row.",
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: "p", text: "While advancing, a perfect line holds and shoots if anyone in it has opened fire. On reform, the front stops until the rear walks into that line, then the whole line walks at half speed, and a unit joining from behind takes the reform order unless that line already fills every row. A full line keeps its order. A unit walking through it keeps its own order and does not halt, reform, or charge that line. An order reaches a line with an open row only from a unit standing on that open row, never from one sharing a row. A charger puts a line onto charge only when that line is reforming and still has an open row. A retreat does not spread to matching units it lines up with." },
    ],
    draw: drawLines,
  },
  {
